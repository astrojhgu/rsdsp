...
{
    /// filters for even channels
    filter_even: BatchFilterFixed<R, T>,

    /// filters for odd channels
    filter_odd: BatchFilterFixed<Complex<T>, T>,
    ...
    Complex<T>: Copy + std::convert::From<R> + Sum + Default + ScalarOperand,
{
    ...
    pub fn new(nch_total: usize, coeff: ArrayView1<T>) -> Self {
        ...
        let coeff = coeff
            ...
            .to_owned();
        let coeff = coeff.slice(s![..;-1,..]);
        let filter_even = BatchFilterFixed::new(coeff);
        let filter_odd = BatchFilterFixed::<Complex<T>, T>::new(coeff);
